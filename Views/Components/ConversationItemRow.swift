...
/**
 * 定义本文件使用的DisplayConversation结构体
 * 避免与其他模块的同名结构体冲突
 */
struct LocalDisplayConversation: Identifiable {
    let id: String
...
struct ConversationItemRow: View {
    /// 会话数据
    var conversation: LocalDisplayConversation
...
#Preview {
    VStack(spacing: 12) {
        ConversationItemRow(
            conversation: LocalDisplayConversation(
                id: "1",
