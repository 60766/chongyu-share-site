...
class HistoricalFigureSelectionViewModel: ObservableObject {
    ...
    private func extractAuthorFromPostId(postId: String) {
        ...
        #if DEBUG
        debugLog("未从postId中找到匹配的作者，设置默认作者")
        #endif
    ...
    private func setPostAuthorByName(authorName: String) {
        #if DEBUG
        debugLog("根据名称设置帖子作者: \(authorName)")
        #endif
        ...
        if let author = allCharacters.first(where: { $0.name == authorName }) {
            ...
            #if DEBUG
            debugLog("找到匹配的作者: \(author.name), 是否在关系字典中: \(characterRelations[author.name] != nil)")
            #endif
            
            if let relations = characterRelations[author.name] {
                #if DEBUG
                debugLog("作者 \(author.name) 的相关角色: \(relations.joined(separator: ", "))")
                #endif
            }
        } else {
            #if DEBUG
            debugLog("警告：未找到名为 \(authorName) 的角色，尝试设置默认作者")
            #endif
            if let defaultAuthor = allCharacters.first(where: { $0.name == "岳飞" }) {
                self.postAuthor = defaultAuthor
            } else if !allCharacters.isEmpty {
    ...
    private func loadPostData() {
        ...
        if postAuthor == nil {
            #if DEBUG
            debugLog("警告：postAuthor为nil，尝试设置默认作者")
            #endif
        ...
        // 调试日志
        #if DEBUG
        debugLog("加载帖子数据 - 作者: \(postAuthor?.name ?? "未知"), 是否在关系字典中: \(postAuthor != nil && characterRelations[postAuthor!.name] != nil)")
        #endif
    ...
    private func loadAllCharacters() {
        ...
        if let url = Bundle.main.url(forResource: "characters", withExtension: "json"),
           let data = try? Data(contentsOf: url) {
            #if DEBUG
            debugLog("characters.json文件大小: \(data.count) 字节")
            #endif
            ...
            do {
                ...
                if let dict = json as? [String: Any],
                   let characters = dict["characters"] as? [[String: Any]] {
                    #if DEBUG
                    debugLog("characters.json包含\(characters.count)个角色 (JSONSerialization验证)")
                    #endif
                ...
                allCharacters = characterData.characters
                        #if DEBUG
                        debugLog("成功从characters.json加载了\(allCharacters.count)个角色")
                        #endif
                        ...
                        if !allCharacters.isEmpty {
                            ...
                            #if DEBUG
                            debugLog("加载的角色示例: \(sampleNames)")
                            #endif
                            return
                        }
                    } catch let decoderError {
                        #if DEBUG
                        debugLog("使用JSONDecoder解析characters.json失败: \(decoderError)")
                        #endif
                        ...
                        for (index, characterDict) in characters.enumerated() {
                            do {
                                ...
                                #if DEBUG
                                debugLog("解析第\(index)个角色失败: \(error)")
                                #endif
                                // 继续解析下一个角色
                            }
                        }
                        ...
                        if !parsedCharacters.isEmpty {
                            #if DEBUG
                            debugLog("成功手动解析了\(parsedCharacters.count)个角色")
                            #endif
                ...
                #if DEBUG
                debugLog("使用JSONSerialization验证characters.json失败: \(jsonError)")
                #endif
            }
        }
        ...
        for fileName in backupFileNames {
            #if DEBUG
            debugLog("尝试加载备用文件\(fileName).json...")
            #endif
            if let url = Bundle.main.url(forResource: fileName, withExtension: "json"),
               let data = try? Data(contentsOf: url) {
                ...
                do {
                    ...
                    allCharacters = characterData.characters
                    #if DEBUG
                    debugLog("成功从\(fileName).json加载了\(allCharacters.count)个角色")
                    #endif
                ...
                #if DEBUG
                debugLog("无法找到或读取\(fileName).json文件")
                #endif
            }
        }
        ...
        if !charactersLoaded {
            #if DEBUG
            debugLog("所有角色JSON文件加载失败，使用硬编码的备用数据")
            #endif
    ...
    func loadHistoricalFigures() {
        ...
        // 检查是否成功加载了角色
        #if DEBUG
        debugLog("从JSON加载的角色数量: \(allCharacters.count)")
        #endif
        
        // 如果角色数量太少，可能是加载失败，尝试使用备用数据
        if allCharacters.count < 50 {
            #if DEBUG
            debugLog("警告：从JSON加载的角色数量太少，可能是加载失败，尝试使用备用数据")
            #endif
            loadFallbackCharacters()
            #if DEBUG
            debugLog("使用备用数据后的角色数量: \(allCharacters.count)")
            #endif
        }
        ...
        loadPostData()
        #if DEBUG
        debugLog("当前帖子作者: \(postAuthor?.name ?? "未设置")")
        #endif
        ...
        // 添加调试日志
        #if DEBUG
        debugLog("历史人物数据加载完成 - 总角色数: \(availableFigures.count), 相关角色数: \(relevantFigures.count)")
        #endif
        
        // 检查是否成功加载了角色
        if availableFigures.isEmpty {
            #if DEBUG
            debugLog("警告：没有加载到任何角色！")
            #endif
        } else {
            // 打印一些角色名称作为验证
        ...
        for name in ["庄子", "老子", "孟子", "爱因斯坦", "莎士比亚"] {
            if let figure = availableFigures.first(where: { $0.name == name }) {
                ...
                #if DEBUG
                debugLog("角色 \(name) 是否在可用角色列表中: \(true), 是否被标记为相关: \(isMarkedAsRelevant)")
                #endif
            } else {
                #if DEBUG
                debugLog("角色 \(name) 是否在可用角色列表中: \(false)")
                #endif
            }
        }
        ...
        if let author = postAuthor {
            #if DEBUG
            debugLog("当前作者: \(author.name), 是否在关系字典中: \(characterRelations[author.name] != nil)")
            #endif
        } else {
            #if DEBUG
            debugLog("当前没有作者")
    ...
    private func buildRelevantFiguresNetwork() {
        ...
        guard let currentAuthor = postAuthor else { 
            #if DEBUG
            debugLog("构建关系网络失败：没有当前作者")
            #endif
            return 
        }
        
        #if DEBUG
        debugLog("开始构建关系网络 - 当前作者: \(currentAuthor.name)")
        #endif
        ...
        if let authorRelations = characterRelations[currentAuthor.name] {
            #if DEBUG
            debugLog("作者 \(currentAuthor.name) 在 characterRelations 中定义的关系: \(authorRelations.joined(separator: ", "))")
            #endif
        } else {
            #if DEBUG
            debugLog("警告：作者 \(currentAuthor.name) 在 characterRelations 中没有定义关系")
            #endif
        }
        ...
        #if DEBUG
        debugLog("直接相关角色数量: \(directlyRelatedCharacters.count)")
        #endif
        ...
        for character in directlyRelatedCharacters {
            if let figure = availableFigures.first(where: { $0.name == character.name }) {
                ...
                #if DEBUG
                debugLog("警告：未找到相关角色 \(character.name) 对应的 CommentHistoricalFigure")
                #endif
            }
        }
        ...
        if authorHasDefinedRelations {
            ...
            #if DEBUG
            debugLog("二级相关角色数量: \(secondaryRelations.count)")
            #endif
            ...
            if !processedRelevantIds.contains(figure.id) {
                ...
                if let character = allCharacters.first(where: { $0.name == figure.name }) {
                    ...
                    if isRelated {
                        ...
                        processedNames.insert(character.name)
                        #if DEBUG
                        debugLog("通过双向关系检查，标记 \(character.name) 为相关角色")
                        #endif
                    }
                }
            }
            ...
            if relatedNames.contains(currentAuthor.name) {
                ...
                if let figure = availableFigures.first(where: { $0.name == characterName }),
                   !processedRelevantIds.contains(figure.id) {
                    ...
                    #if DEBUG
                    debugLog("通过反向关系检查，标记 \(characterName) 为相关角色")
                    #endif
                }
            }
        }
        ...
        #if DEBUG
        debugLog("关系网络构建完成 - 相关角色数量: \(relevantFigures.count)")
        #endif
        #if DEBUG
        debugLog("已处理的相关角色名称: \(processedNames.joined(separator: ", "))")
        #endif
        ...
        for name in ["庄子", "老子", "孟子", "康熙皇帝"] {
            if let figure = availableFigures.first(where: { $0.name == name }) {
                let isMarkedAsRelevant = relevantFigures.contains(figure.id)
                #if DEBUG
                debugLog("角色 \(name) 是否被标记为相关: \(isMarkedAsRelevant)")
                #endif
            } else {
                #if DEBUG
                debugLog("警告：未找到角色 \(name) 对应的 CommentHistoricalFigure")
                #endif
            }
        }
    }
    ...
    private func checkRelationship(between character1: AppCharacter, and character2: AppCharacter) -> Bool {
        ...
        if let relatedNames = characterRelations[character1.name], relatedNames.contains(character2.name) {
            #if DEBUG
            debugLog("发现关系: \(character1.name) 与 \(character2.name) 相关")
            #endif
            return true
        }
        ...
        if let relatedNames = characterRelations[character2.name], relatedNames.contains(character1.name) {
            #if DEBUG
            debugLog("发现关系: \(character2.name) 与 \(character1.name) 相关")
            #endif
            return true
        }
    ...
    private func getDirectlyRelatedCharacters(for character: AppCharacter) -> [AppCharacter] {
        ...
        if let relatedNames = characterRelations[character.name] {
            #if DEBUG
            debugLog("获取 \(character.name) 的相关角色 - 在字典中定义的关系: \(relatedNames.joined(separator: ", "))")
            #endif
            ...
            for name in relatedNames {
                if let relatedCharacter = allCharacters.first(where: { $0.name == name }) {
                    ...
                    #if DEBUG
                    debugLog("警告：未找到相关角色 \(name) 对应的 AppCharacter")
                    #endif
                }
            }
        } else {
            #if DEBUG
            debugLog("警告：\(character.name) 在 characterRelations 中没有定义关系")
            #endif
            ...
            for (characterName, relatedNames) in characterRelations {
                if relatedNames.contains(character.name) {
                    if let relatedCharacter = allCharacters.first(where: { $0.name == characterName }) {
                        relatedCharacters.append(relatedCharacter)
                        #if DEBUG
                        debugLog("通过反向关系找到相关角色: \(characterName)")
                        #endif
                    }
                }
            }
        ...
        #if DEBUG
        debugLog("获取到 \(character.name) 的相关角色数量: \(relatedCharacters.count)")
        #endif
    ...
    func filteredFigures(searchText: String, category: String) -> [CommentHistoricalFigure] {
        ...
        if availableFigures.isEmpty {
            #if DEBUG
            debugLog("警告：availableFigures为空，无法进行筛选")
            #endif
            return []
        }
        ...
        // 打印当前可用角色总数，帮助调试
        #if DEBUG
        debugLog("当前可用角色总数: \(availableFigures.count)")
        #endif
        ...
        if category != "全部" {
            if category == "最近" {
                ...
                if let selectedCategory = CharacterCategory(rawValue: category) {
                    filtered = filtered.filter { figure in
                        ...
                        if let character = allCharacters.first(where: { $0.name == figure.name }) {
                            ...
                            if selectedCategory == .myCreation {
                                debugLog("筛选我的创建分类 - 角色: \(character.name), ID: \(character.id), type: \(character.type), 识别为: \(characterCategory)")
                            }
                            #endif
                    ...
                    #if DEBUG
                    if selectedCategory == .myCreation {
                        debugLog("我的创建分类筛选结果: \(filtered.count) 个角色")
                    }
                    #endif
                }
            }
            ...
            #if DEBUG
            debugLog("选择了'全部'分类，显示所有\(filtered.count)个角色")
            #endif
        }
        ...
        if !searchText.isEmpty {
            ...
            #if DEBUG
            debugLog("应用搜索过滤后的角色数量: \(filtered.count)，搜索文本: \(searchText)")
            #endif
        }
        
        // 打印过滤后的角色数量
        #if DEBUG
        debugLog("过滤后的角色数量: \(filtered.count)，类别: \(category)，搜索文本: \(searchText)")
        #endif
        ...
        #if DEBUG
        debugLog("最终返回角色数量: \(result.count)")
        #endif
    ...
    private func loadUserCreatedCharacters() {
        ...
        do {
            if let characterDicts = try JSONSerialization.jsonObject(with: data) as? [[String: Any]] {
                ...
                for dict in characterDicts {
                    if let id = dict["id"] as? String,
                       ...
                       let era = dict["era"] as? String {
                        ...
                        #if DEBUG
                        debugLog("加载用户创建的角色: \(name), ID: \(characterId), type: custom")
                        #endif
                    }
                }
                ...
                #if DEBUG
                debugLog("成功加载了\(loadedCharacters.count)个用户创建的角色")
                #endif
            }
        } catch {
            #if DEBUG
    ...
    func oneClickInvite() {
        ...
        if figureCount > 0 {
            ...
            #if DEBUG
            debugLog("一键选择：选择了\(figureCount)个相关角色")
            #endif
        } else {
            // 如果没有相关人物，尝试选择关注的角色
            ...
            if !followedFigures.isEmpty {
                ...
                #if DEBUG
                debugLog("一键选择：选择了\(followCount)个关注角色")
                #endif
            } else {
                ...
                #if DEBUG
                debugLog("一键选择：随机选择了\(randomCount)个角色，使用帖子ID作为种子")
                #endif
            }
        }
    }
    ...
    func inviteSelectedFigures() {
        ...
        #if DEBUG
        debugLog("🚀 邀请历史人物参与讨论: \(selectedFigures.map { $0.name }.joined(separator: ", "))")
        #endif
        
        // 获取帖子内容并检查是否有效
        if let content = getPostContent() {
            #if DEBUG
            debugLog("📝 帖子内容长度: \(content.count)字")
            #endif
        } else {
            #if DEBUG
            debugLog("⚠️ 无法获取帖子内容")
        ...
        if !invalidCharacterNames.isEmpty {
            #if DEBUG
            debugLog("⚠️ 无法获取以下人物的有效ID: \(invalidCharacterNames.joined(separator: ", "))")
            #endif
        }
        ...
        #if DEBUG
        debugLog("📣 邀请角色: \(characterIDs.joined(separator: ", ")), 帖子ID: \(postId)")
        #endif
        ...
        if let author = postAuthor {
            ...
            #if DEBUG
            debugLog("📝 虚拟角色发布的帖子，作者: \(author.name)")
            #endif
        } else {
            ...
            if let post = viewModel.posts.first(where: { $0.id.uuidString == postId }) {
                ...
                #if DEBUG
                debugLog("📝 用户发布的帖子，作者: \(post.username)")
                #endif
            } else {
                postAuthorName = nil
                #if DEBUG
                debugLog("⚠️ 未找到帖子，无法获取作者信息")
                #endif
            }
        }
    ...
    func getCharacterIdByName(_ name: String) -> String? {
        #if DEBUG
        debugLog("🔍 HistoricalFigureSelectionViewModel.getCharacterIdByName - 名称: \(name)")
        #endif
        ...
        for character in allCharacters {
            if character.name == name {
                #if DEBUG
                debugLog("✅ 在角色列表中找到: \(name) -> \(character.id)")
                #endif
                return character.id
            }
        }
