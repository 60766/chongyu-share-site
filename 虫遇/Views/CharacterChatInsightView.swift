...
struct CharacterChatInsightView: View {
    ...
    private func loadCachedInsight() {
        // 只加载缓存，不自动生成
        #if DEBUG
        debugLog("🔍 CharacterChatInsightView - 尝试加载缓存画像，角色ID: \(characterId)")
        #endif
        ...
        if let cachedInsight = cachedInsight {
            #if DEBUG
            debugLog("✅ CharacterChatInsightView - 成功加载缓存画像: \(cachedInsight.title)")
            #endif
            insight = cachedInsight
        } else {
            #if DEBUG
            debugLog("⚠️ CharacterChatInsightView - 未找到缓存画像")
            #endif
            insight = nil
        }
    }
