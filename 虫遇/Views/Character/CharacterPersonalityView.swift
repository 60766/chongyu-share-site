...
class CharacterPersonalityViewModel: ObservableObject {
    ...
    func loadPersonality() {
        if let personality = personalityManager.getUserAdjustments(for: characterId) {
            ...
            #if DEBUG
            debugLog("✅ 加载了用户调整的个性化设置: \(characterName)，已调整参数: \(adjustedParameters)")
            #endif
        } else {
            // 用户没有调整，使用默认值(0.5)
            intimacy = 0.5
            ...
            adjustedParameters.removeAll()
            #if DEBUG
            debugLog("📝 使用默认个性化设置: \(characterName)")
            #endif
        }
    }
    ...
    func updatePersonality() {
        ...
        #if DEBUG
        debugLog("🔄 更新个性化参数，只包含调整的参数: \(adjustedParameters)")
        #endif
    }
    ...
    func saveAndApply() {
        updatePersonality()
        #if DEBUG
        debugLog("✅ 已保存并应用角色个性化设置: \(characterName)，调整的参数: \(adjustedParameters)")
        #endif
        }
