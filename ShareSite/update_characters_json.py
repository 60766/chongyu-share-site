...
for character in missing_characters:
    # 检查角色是否已存在
    exists = any(c['id'] == character['id'] for c in characters_data['characters'])
    if not exists:
        characters_data['characters'].append(character)
        print(f"✅ 已添加角色: {character['name']} ({character['id']})")
    else:
        print(f"⚠️ 角色已存在: {character['name']} ({character['id']})")

# 保存更新后的文件
with open(characters_path, 'w', encoding='utf-8') as f:
    json.dump(characters_data, f, ensure_ascii=False, indent=2)

print(f"✅ 已更新characters.json文件，现有角色数量: {len(characters_data['characters'])}") 